...
std::string BigRational::toString()
{
    BigRational number(numerator, denominator);
    BigInteger gd = gcd(number.numerator, number.denominator);
    number.simplify();
    if (number.denominator == 1)
    {
        return number.numerator.toString(); // Если знаменатель равен 1, возвращаем только числитель
    }
    else
    {
...
// ПРИВЕДЕНИЕ К DOUBLE
double BigRational::to_double()
{
    /*
    BigRational num(numerator, denominator);
    std::string res = this->asDecimal(8);
    std::cout << res << "\n";
    double result = (double)std::stod(res);
    std::cout << result << "\n";
    // Преобразуем числитель и знаменатель к типу double перед делением
    // double result = static_cast<double>(numerator) / static_cast<double>(denominator);
    // return result;
    return result;
    */
    std::string res = this->asDecimal(10);
