...
{
    negaFlag(num[0] == '-') // Определение отрицательности числа по наличию знака минуса в строке{
    if (negaFlag) {
    num.erase(0, 1); // Если число отрицательное, удаление знака минуса из строки
    }
    while (num[0] == '0' && num.length() != 1)
    {
        num.erase(0, 1); // Удаление ведущих нулей из числа
        }
    numBI = num;
}
*/
...
BigInteger operator/(const BigInteger &left, const BigInteger &right)
{
    if (right == BigInteger(0))
    {
        ...
        // Здесь можно выбросить исключение, или вернуть какой-то стандартный объект BigInteger,
        // например, BigInteger(0), чтобы обозначить ошибку
        return BigInteger();
    }

    BigInteger quotient;         // Частное
    BigInteger remainder = left; // Остаток от деления

    // Устанавливаем знак частного
    quotient.sign = (left.sign != right.sign);
    ...
    while (remainder >= right)
    {
        ...
        // Умножаем делитель на множитель, пока он меньше или равен остатку
        while (temp * BigInteger(2) <= remainder)
        {
            temp *= BigInteger(2);
            multiple *= BigInteger(2);
        }

        // Делим остаток на полученное значение делителя
        quotient += multiple;
        remainder -= temp;
    }
